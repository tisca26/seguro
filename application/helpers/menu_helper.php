...
if (!function_exists('top_lang_sel')) {
    function top_lang_sel()
    {
        ...
        $result .= '<img src="' . cdn_assets() . 'images/icons/flags/' . $actual_lang .'.png" alt="' . $langs[$actual_lang] . '"></a>';
        $result .= '<ul>';

        foreach ($langs as $key => $lang){
            if ($key != $actual_lang){
                $result .= '<li><a href="' . cambia_idioma($key) . '">';
                $result .= '<img style="height:11px;" src="' . cdn_assets() . 'images/icons/flags/' . $key .'.png" alt="' . $langs[$key] . '">' . $key . '</a></li>';
            }
        }
        $result .= '</ul></li>';
        return $result;
